# Get data from Latin America Databases
...
url <- "redalyc/sustancias_psiactivas/Buscador texto completo Redalyc_page4.html"
urls <- list.files("redalyc/deteccion_temprana/", pattern = ".htm")

resultTotal <- data.frame()
for (i in 1:length(urls)){
  url <-paste0("redalyc/deteccion_temprana/",urls[i])

    red_title <- url %>%
    read_html() %>%
    html_nodes(".resultado-articulo") %>%
    html_text()
  red_title <- matrix(redalyc_author, ncol = 2, byrow = 2)[, 1]

  # red_author <- url %>%
  #   read_html() %>%
  #   html_nodes("a.autor-resultado") %>%
  #   html_text()
  # 
  result <- data.frame(title = red_title)
  resultTotal <- rbind(resultTotal, result)
  
}

write.csv(resultTotal, "resultados_redalyc_dt.csv")